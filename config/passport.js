	...
	passport.use(
		...
		new LocalStrategy(
			...
			(req, email, password, done) => {
				...
				req
					.checkBody('password', "Please enter a valid password that's at least 6 characters long.")
					.notEmpty()
					.isAlpha()
					.isLength({ min: 6 });
				req
					.checkBody('password', 'Please make sure your two passwords match.')
					...
					User.findOne({ email: email }, (err, user) => {
						if (user) {
							...
							return done(
								null,
								false,
								req.flash('signupMessage', 'Nice try, but that email is already taken.')
							);
						}
						...
						User.findOne({ email: email }, (err, user) => {
							...
							if (!user.validPassword(password)) {
								return done(
									null,
									false,
									req.flash('loginMessage', 'Your password looks a bit off. Please try again.')
								);
							}
							// SUCCESSFUL LOGIN!
