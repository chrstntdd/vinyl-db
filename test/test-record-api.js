...
describe('The API', () => {
	...
	describe('*GET endpoint', () => {
		it('should return all records for a given user', () => {
			...
			return User.findOne().exec().then((userObject) => {
				...
				return chai.request(app).get(`/records/${userId}`).then((res) => {
		...
		it('should return the record specified by the id param', () => {
			...
			return User.findOne().exec().then((userObject) => {
				...
				return chai.request(app).get(`/records/${userId}/${recordId}`).then((res) => {
					res.should.have.status(200);
	...
	describe('*POST endpoint', () => {
		it("should post a new record to a given user's music collection", () => {
			...
			return User.findOne().exec().then((userObject) => {
				userId = userObject.id;
				return chai.request(app).post(`/records/${userId}`).send(newRecord).then((res) => {
		...
		it("should update a record's details for a given user", () => {
			...
			return User.findOne()
				...
				.then((userObject) => {
					...
					return chai.request(app).put(`/records/${userId}/${recordId}`).send(updatedRecord);
				})
				.then((res) => {
					res.should.have.status(201);
					return User.findOne().exec();
		...
		it("should increment a particular record's play count", () => {
			...
			return User.findOne()
				...
				.then((userObject) => {
					...
					return chai.request(app).patch(`/records/${userId}/${recordId}`);
				})
				.then((res) => {
					res.should.have.status(200);
