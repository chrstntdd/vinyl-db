module.exports = function(app, passport) {
  ...
  router.delete('/records/:userId/:id', (req, res) => {
    User.findById(req.params.userId)
      ...
      .then(() => {
        const message = `Successfully deleted the record with an id of ${req
          .params.id}`;
        logger.info(message);
        res
          .status(204)
  ...
  router.put('/records/:userId/:id', (req, res) => {
    User.findById(req.params.userId)
      ...
      .then(updatedRecord => {
        const message = `Successfully updated the record with an id of ${req
          .params.id}`;
        logger.info(message);
    ...
    db
      ...
      .then(release => {
        if (release.results.length > 0) {
          ...
          req.flash(
            'no-results',
            "Sorry, we couldn't find that album. Please try your search again."
          );
          res.end();
        }
      })
